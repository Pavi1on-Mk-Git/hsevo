...
Card::Card(
    ...
    mana_cost(default_mana_cost), on_play(on_play), create_play_actions(create_play_actions), keywords(keywords)
{}
...
Card::Card(
    ...
    name(name),
    mana_cost_(base_cost), base_health(base_health), attack(base_attack), health(base_health),
    mana_cost(default_mana_cost), on_play(default_on_play), create_play_actions(default_create_play_actions),
    keywords(keywords)
{}

Card::Card(
    const std::string& name, unsigned base_cost, unsigned base_attack, unsigned base_health,
    const std::function<unsigned(const Card&, const Game&)> mana_cost, const MinionKeywords& keywords
):

    name(name),
    mana_cost_(base_cost), base_health(base_health), attack(base_attack), health(base_health), mana_cost(mana_cost),
    on_play(default_on_play), create_play_actions(default_create_play_actions), keywords(keywords)
{}
