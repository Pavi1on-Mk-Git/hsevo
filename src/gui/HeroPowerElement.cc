...
static const float HERO_POWER_TEXT_HEIGHT_RATIO = 0.1f;
...
void HeroPowerElement::draw_(const Game& game) const
{
    ...
    const raylib::Vector2 stat_rect_size(
        hero_power_rect.width / STAT_BOX_WIDTH_RATIO, hero_power_rect.height / STAT_BOX_HEIGHT_RATIO
    );
    ...
    draw_text(std::to_string(hero->hero_power_mana_cost), text_height * TEXT_HEIGHT_MULTIPLIER, mana_rect);
}