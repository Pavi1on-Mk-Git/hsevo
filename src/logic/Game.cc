...
void Game::draw(unsigned amount)
{
    ...
    current_player().hero.hand.add_cards(std::move(drawn_cards));
    current_player().hero.fatigue(fatigue_count);
}

void Game::draw()
{
    auto drawn_card = current_player().hero.deck.draw();
    if(drawn_card)
        current_player().hero.hand.add_cards(std::move(drawn_card));
    else
...
std::vector<std::unique_ptr<Action>> Game::get_possible_actions()
{
    ...
    {
        auto& current_card = current_player().hero.hand.get_card(hand_position);
...
std::vector<std::unique_ptr<Action>> Game::get_attack_actions()
{
    ...
    for(unsigned opponent_board_position = 0; opponent_board_position < opponent().hero.board.minion_count();
        ++opponent_board_position)
        if(opponent().hero.board.get_minion(opponent_board_position).keywords & TAUNT)
            taunt_minion_positions.push_back(opponent_board_position);

    for(unsigned current_board_position = 0; current_board_position < current_player().hero.board.minion_count();
        ++current_board_position)
    {
        auto& current_minion = current_player().hero.board.get_minion(current_board_position);
...
HeroInput Game::get_hero_state(unsigned player_index)
{
    ...
    const unsigned board_size = players_.at(player_index).hero.board.minion_count();
    for(unsigned minion_index = 0; minion_index < board_size; ++minion_index)
    {
        auto& curr_minion = players_.at(player_index).hero.board.get_minion(minion_index);
    ...
    return HeroInput{
        hero_hero, minion_heros, players_.at(player_index).hero.hand.size(), board_size,
